          ...
          {deals.map((d) => (
            <motion.div key={d.customer} variants={fadeUp} className="monolith-card p-6 transition-all duration-300">
              {'logos' in d && d.logos ? (
                ...
                <img src={d.logo} alt={d.customer} className={`object-contain mb-4 opacity-70 ${d.customer === 'Arcese for Ferrari' ? 'h-10' : d.customer === 'TransmecLog' ? 'h-14' : d.customer === 'Luxottica' || d.customer === 'FORTNA / MrPrice' ? 'h-20' : 'h-16'}`} />
              ) : null}
